...
vector<uint64_t> calc_product_diagonal(int64_t offset, uint64_t colsize_R, uint64_t rowsize_R, vector<uint64_t> kernel_L, vector<uint64_t> kernel_L_indexes, vector<uint64_t> list_R, Modulus & modulus){
    ...
    for(uint64_t i = 0;i < kernel_L_indexes.size();i++){
        uint64_t prod;
        if(offset >= 0){
            if(kernel_L_indexes[i]+offset >= list_R.size() || kernel_L_indexes[i] >= kernel_L.size()) continue;
            prod = util::multiply_uint_mod(kernel_L[kernel_L_indexes[i]], list_R[kernel_L_indexes[i]+offset], modulus);
        }else{
            if(kernel_L_indexes[i]-offset >= kernel_L.size() || kernel_L_indexes[i] >= list_R.size()) continue;
            prod = util::multiply_uint_mod(kernel_L[kernel_L_indexes[i]-offset], list_R[kernel_L_indexes[i]], modulus);
        }
...
void test_prod_diagonal(){
    ...
    vector<uint64_t> diagonal_0 = calc_product_diagonal(1, colsize_R, rowsize_R, kernel_diagonallist, indexes, list_R, modulus);
